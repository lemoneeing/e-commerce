...
import com.hhplusw03.ecommerce.domain.wallet.infrastructure.WalletRepositoryImpl;
...
@Import({WalletServiceImpl.class, WalletRepositoryImpl.class})
class WalletServiceImplTest {

    @MockBean
    WalletRepositoryImpl repo;
    @Autowired
    ...
    public void 지갑_생성_서비스_성공(){
        ...
        when(repo.saveWallet(Long.parseLong(userId))).thenReturn(walletEntity);
        ...
        verify(repo, times(1)).saveWallet(Long.parseLong(userId));
    }
...
//    }
    ...
    public void 잔액_조회_서비스_성공(){
        ...
        when(repo.readWallet(Long.parseLong(userId))).thenReturn(walletEntity);
        ...
        verify(repo, times(1)).readWallet(Long.parseLong(userId));
    }
...
//        //지갑 데이터 생성
//        when(repo.saveWallet(Long.parseLong(userId))).thenReturn(new WalletEntity(Long.parseLong(userId), Long.parseLong(initBalance)));
...
//        String amountToCharge = "50000";
        ...
        Long userId = 1L;
        Long initBalance = 0L;
        Long amountToCharge = 50000L;
        // 충전 시험
        WalletEntity walletEntity = new WalletEntity(userId, initBalance);
        when(repo.readWallet(userId)).thenReturn(walletEntity);

//        walletEntity.setBalance(initBalance + amountToCharge);
        WalletEntity newWalletEntity = new WalletEntity(userId, amountToCharge);
        when(repo.modifyBalance(walletEntity, initBalance + amountToCharge)).thenReturn(newWalletEntity);

        WalletResult walletRes = svc.chargeWallet(String.valueOf(userId), String.valueOf(amountToCharge));

        Assertions.assertThat(walletRes).isInstanceOf(WalletDto.class);
        Assertions.assertThat(((WalletDto)walletRes).getUserId()).isEqualTo(userId);
        Assertions.assertThat(((WalletDto)walletRes).getBalance()).isEqualTo(amountToCharge);

        verify(repo, times(1)).readWallet(userId);
        verify(repo, times(1)).modifyBalance(walletEntity, initBalance + amountToCharge);
    }
//    @Test
